...
# ---------------------------------------------------------------------------
# Helper: isi satu worksheet per-campaign (format FILTER/OFF FIX)
# Mengembalikan dict akumulasi total per tanggal untuk sheet TOTAL opsional.
# ---------------------------------------------------------------------------

def _fill_campaign_sheet(
    ws,
    camp_id: UUID,
    camp_name: str,
    tag_ids: list,
    ...
    FMT_IDR = "#,##0"
    FMT_PCT = "0.00%"
    TAG_W = 5        # TGL, KOMISI, Klik Shp, CPC Shp, STATUS
    TAG_GAP = 1
    TAG_START = 16
    ...
    def _h(cell, value):
        cell.value = value
        cell.font = _hdr_font_w(); cell.fill = _hdr_fill()
        cell.alignment = Alignment(horizontal="center"); cell.border = _thin_border()

    for ci, label in enumerate(MAIN_COLS, start=1):
        _h(ws.cell(1, ci), label)

    for ti, tag_id in enumerate(tag_ids):
        base = TAG_START + ti * (TAG_W + TAG_GAP)
        ...
        _h(ws.cell(1, base), tag_label)
        for si, slabel in enumerate(TAG_SUB):
            _h(ws.cell(2, base + si), slabel)

    nd = len(dates)
    totals: dict = {}

    for ridx, d in enumerate(dates, start=3):
        m = meta_by.get(camp_id, {}).get(d, {})
        spend = m.get("spend", 0)
        clicks_fp = m.get("clicks", 0)
        komisi_total = sum(shopee_by.get(t, {}).get(d, {}).get("komisi", 0) for t in tag_ids)
        klik_shp_total = sum(shopee_by.get(t, {}).get(d, {}).get("klik", 0) for t in tag_ids)

        totals[d] = {"spend": spend, "komisi": komisi_total,
                     "clicks_meta": clicks_fp, "clicks_shopee": klik_shp_total}
        ...
        if clicks_fp:   ws.cell(ridx, 7, clicks_fp)
        if klik_shp_total: ws.cell(ridx, 8, klik_shp_total)
    ...
    _apply_total_row(ws, tr, 11 + len(tag_ids) * (TAG_W + TAG_GAP))
...
async def _fetch_campaign_data(db, camp_ids, all_tag_ids, tanggal_dari, tanggal_sampai):
    """Fetch meta + shopee metrics, return (meta_by, shopee_by)."""
    meta_rows = (await db.execute(
...
async def export_laporan_filter(
    ...
    total_by_date: dict = defaultdict(lambda: {
        "spend": _ZERO, "komisi": _ZERO, "clicks_meta": 0, "clicks_shopee": 0
    })

    for camp in sorted(campaigns, key=lambda c: c.nama_campaign):
        ws = wb.create_sheet(camp.nama_campaign[:31])
        day_totals = _fill_campaign_sheet(
            ws, camp.id, camp.nama_campaign,
            camp_to_tags.get(camp.id, []),
            tag_name_map, meta_by, shopee_by, dates,
        )
        for d, t in day_totals.items():
            total_by_date[d]["spend"]        += Decimal(str(t["spend"]))
            total_by_date[d]["komisi"]       += Decimal(str(t["komisi"]))
            total_by_date[d]["clicks_meta"]  += t["clicks_meta"]
            total_by_date[d]["clicks_shopee"]+= t["clicks_shopee"]

    # TOTAL FILTER sheet di index 0
    FMT_IDR = "#,##0"
    FMT_PCT = "0.00%"
    TCOLS = ["TGL", "Spend", "(+) 5%", "KOMISI", "PROFIT", "(%) PROFIT",
             "Klik FP", "Klik Shp", "(%) Klik", "CPC FP", "CPC Shp"]
...
@router.get("/laporan-off-fix")
async def export_laporan_off_fix(
    current_user: CurrentUser,
    db: DB,
    tanggal_dari: date = Query(...),
    tanggal_sampai: date = Query(...),
    ...
    q_camp = (
        ...
        .where(
            MetaAccount.user_id == current_user.id,
            Campaign.tahap.in_(["off", "fix_scale_up"]),
        )
    )
    if meta_account_id:
        q_camp = q_camp.where(Campaign.meta_account_id == meta_account_id)
    campaigns = (await db.execute(q_camp)).scalars().all()
    if not campaigns:
        raise HTTPException(404, "Tidak ada campaign dengan tahap Off atau Fix Scale Up.")
    camp_ids = [c.id for c in campaigns]
    ...
    bulan = tanggal_dari.strftime("%B %Y").upper()
    return _stream_wb(wb, f"OFF FIX META {bulan}.xlsx")
