...
export class AnalysisPage {
  ...
  async _exportHarian() {
    const btn = this.container.querySelector('#btn-export-harian');
    const orig = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = 'Memproses…';
    try {
      const qs = filterQS ? filterQS() : '';
      const token = getToken();
      const url = `/api/export/laporan-harian?tanggal_dari=${this.dari}&tanggal_sampai=${this.sampai}${qs}`;
      const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        alert('Export gagal: ' + (err.detail || res.statusText));
  ...
  async _exportOffFix() {
    const btn = this.container.querySelector('#btn-export-off-fix');
    const orig = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = 'Memproses…';
    try {
      const qs = filterQS ? filterQS() : '';
      const token = getToken();
      const url = `/api/export/laporan-off-fix?tanggal_dari=${this.dari}&tanggal_sampai=${this.sampai}${qs}`;
      const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        alert('Export gagal: ' + (err.detail || res.statusText));
      ...
      const cd = res.headers.get('Content-Disposition') || '';
      const match = cd.match(/filename="?([^"]+)"?/);
      a.download = match ? match[1] : 'OFF FIX META.xlsx';
      a.click();
      URL.revokeObjectURL(a.href);
    } catch(e) {
      alert('Export gagal: ' + e.message);
  ...
  async _exportFilter() {
    const btn = this.container.querySelector('#btn-export-filter');
    const orig = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = 'Memproses…';
    try {
      const qs = filterQS ? filterQS() : '';
      const token = getToken();
      const url = `/api/export/laporan-filter?tanggal_dari=${this.dari}&tanggal_sampai=${this.sampai}${qs}`;
      const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        alert('Export gagal: ' + (err.detail || res.statusText));
      ...
      const cd = res.headers.get('Content-Disposition') || '';
      const match = cd.match(/filename="?([^"]+)"?/);
      a.download = match ? match[1] : 'FILTER META.xlsx';
      a.click();
      URL.revokeObjectURL(a.href);
    } catch(e) {
      alert('Export gagal: ' + e.message);
