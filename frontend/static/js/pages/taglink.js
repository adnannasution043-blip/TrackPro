...
export class TaglinkPage {
  ...
  _renderTable(el) {
    ...
    if (pgWrap) {
      ...
      pgWrap.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 4px;margin-top:8px;flex-wrap:wrap;gap:8px;">
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-size:12px;color:var(--text-muted,#9ca3af);">per halaman &nbsp;·&nbsp; ${startIdx+1}–${Math.min(startIdx+pp, allRows.length)} dari ${allRows.length}</span>
          </div>
      ...
      pgWrap.querySelector('#pg-size')?.addEventListener('change', e=>{ this._perPage=Number(e.target.value); this._page=1; this._renderTable(el); });
      ...
      pgWrap.querySelectorAll('.pg-num').forEach(btn=>{ btn.addEventListener('click',()=>{ this._page=Number(btn.dataset.pg); this._renderTable(el); }); });
    }
  }
